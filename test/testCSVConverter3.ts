...
describe("testCSVConverter3", () => {
    afterEach(() => {
        sb.restore();
    });
    it("should parse large csv file with UTF-8 without spliting characters", (done) => {
        const testData = __dirname + "/data/large-utf8.csv";
        const rs = fs.createReadStream(testData);
        const csvConverter = csv({});
        const count = 0;
        csvConverter.preRawData((csvRawData) => {
            assert(csvRawData.charCodeAt(0) < 2000);
            return csvRawData;
        });
        csvConverter.on("data", () => {
            count++;
        });
        csvConverter.then(() => {
            assert(count === 5290);
    ...
    it("should setup customise type convert function", (done) => {
        csv({
            checkType: true,
            colParser: {
                column1: "string",
                column5: (item, head, resultRow, row, i) => {
    ...
    it("should accept pipe as quote", (done) => {
        csv({
            quote: "|",
            output: "csv",
        })
    ...
    it("emit file not exists error when try to open a non-exists file", () => {
        const called = false;
        const cb = sb.spy((err) => {
...
1,2`)
  ...
  "fefe,5",6`)
    ...
    it("should parse header with quotes correctly", () => {
    ...
    it("should stream json string correctly", (done) => {
        const data = `a,b,c
1,2,3
4,5,6`;
    ...
    it("should stream json line correctly", (done) => {
        const data = `a,b,c
1,2,3
4,5,6`;
        csv({
    ...
    it("should not send json if needEmitAll is false", async () => {
        const data = `a,b,c
1,2,3
4,5,6`;
        return csv({
    ...
    it("should convert null to null object", async () => {
        const data = `a,b,c
null,2,3
4,5,6`;
        return csv({
    ...
    it("should process period properly", async () => {
        const data = `a..,b,c
1,2,3
4,5,6`;
        return csv({})
