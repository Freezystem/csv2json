 ...
 * 2. check if utf8 chars being split, if it does, stripe the bytes and add to the left buffer.
 * 3. stripBom
 */
...
/**
 *  append data to the buffer that is left from the last chunk
 */
...
/**
 * check if utf8 chars being split, if it does, stripe the bytes and add to the left buffer.
 */
function cleanUtf8Split(chunk: Buffer, runtime: ParseRuntime): Buffer {
     ...
     * If the char being split, the buffer from two chunks needs to be concat
