...
describe("Test delimiters", () => {
    ...
    it("should return the explicitly specified delimiter", () => {
        const delimiter = ";";
        const rowStr = "a;b;c";
    ...
    it("should return the autodetected delimiter if 'auto' specified", () => {
        const rowStr = "a;b;c";
        const returnedDelimiter = getDelimiter(rowStr, { delimiter: "auto" });
        assert(returnedDelimiter === ";");
    });

    it("should return the ',' delimiter if delimiter cannot be specified, in case of 'auto'", () => {
        const rowStr = "abc";
        const returnedDelimiter = getDelimiter(rowStr, { delimiter: "auto" });
        assert(returnedDelimiter === ",");
    });

    it("should accept an array with potential delimiters", () => {
        const rowStr = "a$b$c";
        const returnedDelimiter = getDelimiter(rowStr, {
            delimiter: [",", ";", "$"],
        });
        assert(returnedDelimiter === "$");
    });
...
describe("ParseMultiLine function", () => {
    ...
    it("should return partial if line not closed", () => {
...
describe("RowSplit.parse function", () => {
    const rowSplit = new RowSplit(new Converter());
    const func = (str): RowSplitResult => {
        return rowSplit.parse(str);
    };
    it("should split complete csv line", () => {
    ...
    it("should split incomplete csv line", () => {
        const str = 'hello,world,"csvtojson,awesome';
        const res = func(str);
        assert.equal(res.closed, false);
    });

    it("should allow multiple line", () => {
